...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    scores_lay1 = np.dot(X, W1) + b1 # (N, H)
    h1 = np.maximum(0, scores_lay1) # (N, H)
    scores_lay2 = np.dot(h1, W2) + b2 # (N, C)
  ...
  def predict(self, X):
    ...
    ###########################################################################
    scores_lay1 = np.dot(X, self.params['W1']) + self.params['b1'] # (N, H)
    h1 = np.maximum(0, scores_lay1) # (N, H)
    scores_lay2 = np.dot(h1, self.params['W2']) + self.params['b2'] # (N, C)
    scores = scores_lay2
